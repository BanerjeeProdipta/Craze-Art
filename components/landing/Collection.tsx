      ...
      <div className="justify-between text-white sm:flex">
        {tabItems.map((singleItem) => (
          <button
            ...
            className={`${
              selected === singleItem ? 'bg-primaryLight' : 'bg-white/20'
            } m-2 w-full whitespace-nowrap rounded-md px-4 py-2 font-medium transition duration-1000 hover:bg-gradient-to-tr lg:px-4`}
          >
            {singleItem}
          </button>
        ))}
